...
cyGLRenderDepth2D depthRenderBuffer;
...
void renderObject()
{
    ...
    GLint mvpLoc = glGetUniformLocation(object_program, "mvp");
    GLint mvLoc = glGetUniformLocation(object_program, "mv");
    GLint viewLoc = glGetUniformLocation(object_program, "view");
    GLint mNormLoc = glGetUniformLocation(object_program, "mNorm");
    GLint modelLoc = glGetUniformLocation(object_program, "model");

    glUniformMatrix4fv(mvpLoc, 1, GL_FALSE, mvp);
    glUniformMatrix4fv(mvLoc, 1, GL_FALSE, mv);
    glUniformMatrix4fv(viewLoc, 1, GL_FALSE, view);
    glUniformMatrix3fv(mNormLoc, 1, GL_FALSE, normal);
...
void setMatrices()
{
    ...
    cy::Matrix4<float> viewMat;

    // float radius = z_distance; // Maintain the distance from the origin
    // float theta = x_drag;      // Horizontal rotation (yaw)
    // float phi = y_drag;        // Vertical rotation (pitch)
    ...
    viewMat.SetView(cy::Vec3f(0.0f, 0.0f, z_distance), cy::Vec3f(0.0f, 0.0f, 0.0f), cy::Vec3f(0.0f, 1.0f, 0.0f));

    cy::Matrix4<float> modelMatrix = rotation_plane;
    cy::Matrix4<float> modelView = viewMat * rotation * modelMatrix;
